...
function (angular, app, $, _, kbn, moment, TimeSeries, PanelMeta) {
  ...
  module.controller('GraphCtrl', function($scope, $rootScope, panelSrv, annotationsSrv, panelHelper, $q) {
    ...
    $scope.panelMeta.addEditorTab('Alert', 'app/features/panel/partials/alertSet.html');
    ...
    $scope.refreshAlerts = function() {
      $scope.panel.alerts = [];
      $scope.panel.alerts.push(panelHelper.getAlertData($scope));

    }

    $scope.removeAlert = function(alert) {
      $scope.panel.alerts = _.without($scope.panel.alerts, alert);
      $scope.render();
    }
    $scope.addAlert = function() {
      $scope.panel.alerts.push({enable:"true", name:"", threshold1:"", threshold2:"", target:""})
      $scope.render();
    }
